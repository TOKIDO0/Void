...
- 历史聊天面板复制、最新用户消息编辑重发、平滑回到底部
- 设置面板语言化输出规模与行星轨道滑杆
- MVP 长文本能力边界澄清：支持长文本，但不把 VOID 定位成 Claude / Codex 替代品
...
- 设置模态框使用半透明玻璃质感。
- 当前优先实现“模型”分组。
- 支持 Provider、API Key、Base URL、Model Name、模型强度、回应风格、输出规模、流式输出开关。
- Provider 支持 `OpenAI-compatible` 和 `Anthropic`。
- 设置中已增加 OpenAI、DeepSeek、豆包 Ark、智谱 GLM、Anthropic Claude 等预设。
...
- “回应风格”和“输出规模”已改为语言化滑杆，不再直接暴露温度和 token 数字。
- 滑杆已改为行星轨道样式，支持克制粒子拖尾，并修正轨道两极越界问题。
...
- 历史聊天面板打开时默认显示最新消息。
- 用户向上滚动后会出现“回到底部”按钮，点击后平滑滚动到最新消息。
- 每条消息支持复制，复制后显示同主题提示气泡。
- 最新用户消息支持编辑后重新发布，新的 assistant 回复会覆盖当前回复，不新增重复记录。
...
- `14_VOID_模型厂商与模型选择谱系文档.md`
  - 看模型厂商、模型 ID、强度映射和中转站边界。
- `15_VOID_下一步开发方案.md`
  - 看当前阶段下一步开发计划。

## 4. 下一步建议

下一步建议做“产品定位文档与 system prompt 对齐 + 长文本承载检查 + 真实模型链路验证”，不要马上做完整语音对话、记忆系统或健康档案。

原因：
- 当前 agent 视觉已经成立。
- 麦克风现在只负责视觉状态，不负责语义理解。
- 如果直接做 STT/TTS，会同时引入权限、转写、唤醒、回复、播放多个不稳定变量。
- 设置模态框、基础模型配置、OpenAI-compatible、Anthropic、常用厂商预设、回复呈现层和历史面板已完成。
- 当前已经明确：VOID 必须支持长文本，但不能定位成 Claude / Codex 替代品。
- 现在需要把 `01` 产品定位、`03` system prompt、文本承载 UI 和模型输出规模策略对齐。
- 随后再验证真实模型请求链路，尤其是 DeepSeek、智谱、豆包 Ark、Anthropic 和用户自定义 Base URL 是否能在开发代理下稳定返回。
- 生产 Web 或 Tauri 阶段不能长期依赖 Vite dev proxy，需要尽快设计正式模型请求代理和 Key 安全边界。

建议下一步任务：
1. 先更新 `03_VOID_System_Prompt正式版.md`，把长文本能力边界写进去：
   - 默认回应克制，不自动长篇。
   - 用户明确要求长文、代码、档案整理时，允许结构化长输出。
   - 情绪陪伴场景优先短句、低压、自然。
2. 检查当前长文本承载是否稳定：
   - 底部输入框输入长文本时不撑坏布局。
   - 主界面回复层只显示克制预览，不把主界面变成聊天列表。
   - 历史面板能舒服阅读长回复，并保留复制、编辑、回到底部功能。
3. 确认模型输出规模策略：
   - 保留 `简短回应 / 常规任务 / 长文/代码 / 档案级输出`。
   - 默认不要选最高档。
   - 高档位用于明确长任务，不在前端写死过低上限。
