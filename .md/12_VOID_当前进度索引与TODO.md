...
下一步建议做“模型配置与文本对话第一闭环”，不要马上做完整语音对话。
...
建议下一步任务：
1. 新增本地模型配置模块。
2. 支持至少 OpenAI-compatible provider。
3. 建立统一 provider contract。
4. 做一个极简隐藏/弱化设置入口。
5. 完成文本输入到 LLM 回复的第一闭环。
6. 对话期间驱动 `thinking` / `speaking` 状态。
...
7. 对话流程要驱动 agent 的 thinking / speaking 状态。
8. 不要添加多余 UI 或测试文案。
9. 不要写测试代码，除非用户明确要求。
```
