...
const TOOL_USE_COMMON_SUFFIX = [
  ...
  "相互独立的只读查询可放在同一轮多个 tool_calls 里并行执行；有依赖的步骤按顺序分轮调用。",
...
const AGENT_TOOL_USE_SUFFIX = [
  "本轮只允许使用 Agent 自检/预演/任务协作/调度工具：agent.inspectCapabilities、agent.planTaskRoute、agent.inspectToolContract、agent.inspectExtensionPolicy、agent.inspectSafetyHooks、agent.inspectPrivacyBoundaries、agent.inspectTaskPlaybooks、agent.inspectSkills、agent.todo、agent.goal、agent.askUser、agent.spawnTask、agent.scheduleCreate、agent.scheduleList、agent.scheduleRemove、agent.scheduleRunNow 或 agent.scheduleInspect。",
  "用户问当前有哪些能力/工具/授权概况时，用 agent.inspectCapabilities；用户问某个任务会用哪些工具、有什么风险、先别执行只看计划时，用 agent.planTaskRoute；用户问某个具体工具的契约、权限、风险、资源、审计或输出来源时，用 agent.inspectToolContract；用户问插件/MCP/skills/hooks/subagents/扩展机制是否启用、安全边界或接入策略时，用 agent.inspectExtensionPolicy；用户问哪些动态安全规则会触发确认、为什么 localhost/.env/私网/敏感文件会升为 L2 时，用 agent.inspectSafetyHooks；用户问哪些数据会离开本机、会不会发云端、记忆/语音/模型上下文隐私边界时，用 agent.inspectPrivacyBoundaries；用户问有哪些任务模板、工作流、playbook、组合任务、用法示例或怎么更高效使用你时，用 agent.inspectTaskPlaybooks；用户问我有哪些技能/技能库/已安装的任务剧本/怎么用技能时，用 agent.inspectSkills 列出本机技能目录；用户要创建定时/到点/周期性后台任务时，用 agent.scheduleCreate（L2 确认一次授 scope，无人值守不再确认），查看用 agent.scheduleList，删除用 agent.scheduleRemove，立即触发用 agent.scheduleRunNow，任务与近期执行一次看清用 agent.scheduleInspect；周期巡检类任务在 prompt 里约定无异常首行只回 NO_NOTIFY（免打扰），有异常正常汇报。",
