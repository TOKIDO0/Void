...
const TOOL_USE_SYSTEM_SUFFIX = [
  ...
  "页面内操作顺序：看不清结构或 selector 不稳 → browser.extract → 再用返回的 suggestedSelector 做 browser.click / browser.type；无 suggestedSelector 时用 text/href 收窄，禁止空猜。",
  "browser.click / browser.type 的 selector 必须唯一匹配；browser.waitFor 用于导航后等待；禁止假装已点击/已输入。",
