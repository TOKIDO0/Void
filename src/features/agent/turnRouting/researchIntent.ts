...
const RESEARCH_ACTION_PATTERN =
  /(?:搜集|收集|检索|调研|查证|核实|求证|查资料|找资料|找信息|找一下|找一找|帮我找|帮忙找|整理资料|汇总一下|汇总下|总结一下|总结下|帮我查|给我查|查一查|查一下|上网查|联网查|网上查|搜一搜|搜一下|帮我搜|给我搜|搜索一下|搜索下)/;

// 「总结一下刚才的聊天」是对话整理不是联网检索：命中对话指代时不算检索动作。
const CONVERSATION_DIGEST_GUARD =
  /(?:刚才|本次|这轮|这一轮|聊天|对话|讨论|会话|这段)/;

const RESEARCH_TOPIC_PATTERN =
  /(?:最火|最热|很火|火的|出圈|刷屏|热门|流行|排行|榜单|最新|近况|进展|现状|趋势|口碑|评价|对比|区别|哪个好|有哪些|是什么|怎么回事|为什么|原因|资料|信息|新闻|消息|报道|最牛|最强|最厉害|最红|最牛逼)/;
...
export function isResearchIntent(userInput: string): boolean {
  ...
  if (RESEARCH_ACTION_PATTERN.test(text)) {
    // 纯对话整理（总结刚才的聊天）不触发检索增强：除非同时有名确的联网/话题信号。
    if (
      /(?:总结一下|总结下|汇总一下|汇总下)/.test(text)
      && CONVERSATION_DIGEST_GUARD.test(text)
      && !/(?:上网|联网|网上|网页|搜索|检索|github|trending|新闻|榜单|排行|热门|最火|很火|最新)/i.test(text)
    ) {
      return false;
    }
    return true;
...
export function buildResearchSourcePromptSuffix(): string {
  return [
    ...
    "6. 汇报用口语中文卡片式来源：每条先一句话结论，再跟标题、1-2 句摘要、完整 http(s) 链接；多条之间用「一、二、三」分段，不要 Markdown 列表符。"
  ].join("\n");
}
