...
const AGENT_WORKSPACE_TOOL_NAMES = [
...
const DESKTOP_SCREENSHOT_PATTERN = /(?:(?:截屏|截图|屏幕截图|桌面截图).{0,12}(?:看看|一下|当前|桌面)?|(?:看看|查看).{0,10}(?:屏幕|桌面).{0,6}(?:内容|当前|截图)?|(?:看看|看一下|看下|截个图).{0,8}(?:这个|屏幕|报错|窗口)|屏幕上)/i;
// AQ 读屏 OCR：识别图中文字才进读屏工具组（截图+识别同轮）
const DESKTOP_OCR_PATTERN =
  /(?:识别|辨认|读取|读出|提取).{0,12}(?:图中|图片|截图|屏幕).{0,8}(?:文字|文本|内容)|(?:图中|截图|屏幕).{0,8}(?:文字|文本).{0,8}(?:是什么|有哪些|识别|提取|读取)/i;
const DESKTOP_OCR_TOOL_NAMES = [
  "desktop.screenshot",
  "desktop.readScreenText"
];
...
const AGENT_SCHEDULE_PATTERN =
  /(?:(?:定时|到点).{0,12}(?:提醒|执行|告诉我|发|检查|播报|汇总)|(?:每天|每小时|每隔|每\d+[分时天周]).{0,16}(?:提醒|告诉我|发给我|检查|执行|播报|汇总)|提醒我.{0,16}(?:每天|每小时|到点|定时)|(?:后台任务|定时任务).{0,12}(?:结果|状态|列表|查看|怎么样|如何|进度)|(?:早报|晨报|日报).{0,12}(?:生成|发|播报|提醒|订)|(?:生成|做|订).{0,12}(?:早报|晨报|日报)|(?:明早|明晚|明天|后天|大后天|下周|下个月|月底).{0,12}(?:提醒|定时|告诉我|执行|开会)|(?:周[一二三四五六日天]|星期[一二三四五六日天]).{0,12}(?:提醒|定时|开会|告诉我)|(?:\d+号).{0,12}(?:提醒|定时)|(?:\d+[分时天]后|半小时后).{0,8}(?:提醒|定时)|(?:周[一二三四五六日天]).{0,6}(?:上午|下午|早上|晚上|中午|凌晨)?\d{1,2}点|cron)/i;
// B 自然语言时间守卫：含查找/文档语义的一律不进调度（防"周三会议纪要在哪"被劫持）
const SCHEDULE_LOOKUP_GUARD =
  /(?:纪要|笔记|文档|文件|总结|查找|搜索|在哪|哪里)/;
...
function classifyDirectCapability(userInput: string): TurnCapabilityRoute {
  ...
  if (AGENT_SCHEDULE_PATTERN.test(userInput) && !SCHEDULE_LOOKUP_GUARD.test(userInput)) {
  ...
  // AQ 读屏 OCR：显式识别语义优先于纯截图
  if (DESKTOP_OCR_PATTERN.test(userInput)) {
    return createRoute("desktop", DESKTOP_OCR_TOOL_NAMES);
  }
  ...
  if (DESKTOP_OPEN_FILE_PATTERN.test(userInput)) {
    return createRoute("desktop", DESKTOP_OPEN_FILE_TOOL_NAMES);
  }
  ...
  const simpleLaunchMatch = DESKTOP_SIMPLE_LAUNCH_PATTERN.exec(userInput);
  if (simpleLaunchMatch) {
    const appName = simpleLaunchMatch[1]?.trim().toLowerCase();
    if (
      appName
      && !DESKTOP_LAUNCH_BLOCKLIST.some((w) => appName.includes(w.toLowerCase()))
      && !userInput.toLowerCase().includes("http")
      && !userInput.includes("://")
