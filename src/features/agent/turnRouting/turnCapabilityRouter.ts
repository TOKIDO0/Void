...
const AGENT_SCHEDULE_PATTERN =
  /(?:(?:定时|到点).{0,12}(?:提醒|执行|告诉我|发|检查|播报|汇总)|(?:每天|每小时|每隔|每\d+[分时天周]).{0,16}(?:提醒|告诉我|发给我|检查|执行|播报|汇总)|提醒我.{0,16}(?:每天|每小时|到点|定时)|(?:后台任务|定时任务).{0,12}(?:结果|状态|列表|查看|怎么样|如何|进度)|(?:早报|晨报|日报).{0,12}(?:生成|发|播报|提醒|订)|(?:生成|做|订).{0,12}(?:早报|晨报|日报)|cron)/i;
